...
d1 <- df %>% 
     filter(Date_reported == ('2022-10-18')) %>% arrange(-Cumulative_deaths)
```
...
df %>% 
  ...
  ggplot(aes(y=Cumulative_deaths,
            x=Date_reported, color=Country_code))+ 
  geom_line()+
...
```{r}
d1 <- read_csv('https://raw.githubusercontent.com/owid/covid-19-data/master/public/data/vaccinations/vaccinations.csv')
...
d1 %>%
  ...
  labs(x='Date',
       ...
       caption = 'Note: The High Income population has the highest total vaccinations followed by Upper Middle.')
